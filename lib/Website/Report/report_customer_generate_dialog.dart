...
class GenerateCustomerReportDialog extends StatefulWidget {
  final String customerId;
  final String customerName;

  const GenerateCustomerReportDialog({
    super.key,
  ...
  @override
  State<GenerateCustomerReportDialog> createState() =>
      _GenerateCustomerReportDialogState();
}

class _GenerateCustomerReportDialogState
    extends State<GenerateCustomerReportDialog> {
    ...
    try {
      ...
      try {
        ...
        customerData = await supabase
            .from('customer')
            .select('name, mobile_number, address, balance_debit')
      ...
      if (_includeOrders) {
        try {
          ...
          dynamic queryBuilder = supabase
              .from('customer_order')
              .select(
                'customer_order_id, order_date, total_balance, order_status',
              )
              .eq('customer_id', customerId)
          ...
          for (var i = 0; i < orders.length; i++) {
            ...
            try {
              ...
              final orderDetails = await supabase
                  ...
                  .select('quantity, total_price, product:product_id(name)')
                  .eq('customer_order_id', order['customer_order_id'])
                  .timeout(const Duration(seconds: 30));
              order['items'] = orderDetails ?? [];
            } catch (e) {
              print(
                'Error fetching order details for order ${order['customer_order_id']}: $e',
              );
              order['items'] = [];
            }
          }
      ...
      try {
        ...
        await _buildPdfPages(pdf, customerData, orders, payments);
  ...
  Future<void> _buildPdfPages(
    ...
    for (var payment in payments['check']!) {
      final checkDetails = payment['customer_checks'];
            ...
            pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  _buildInfoRow('Name:', customerData['name'] ?? 'N/A'),
                  _buildInfoRow(
                    'Mobile:',
                    customerData['mobile_number'] ?? 'N/A',
                  ),
                  _buildInfoRow('Address:', customerData['address'] ?? 'N/A'),
                  _buildInfoRow(
                    'Balance Debit:',
                    '\$${customerData['balance_debit'] ?? '0'}',
                  ),
                ],
              ),
            ),
                  ...
                  children: [
                    ...
                    pw.Row(
                      ...
                      children: [
                        ...
                        pw.Text(
                          '\$${orders.fold<double>(0, (sum, order) => sum + ((order['total_balance'] as num?)?.toDouble() ?? 0)).toStringAsFixed(2)}',
                          style: pw.TextStyle(
                            fontSize: 13,
                            fontWeight: pw.FontWeight.bold,
                            color: PdfColors.blue900,
          ...
          children: [
            ...
            ...items.map((item) {
              ...
              final price =
                  ...
                  ? (item['total_price'] / item['quantity']).toStringAsFixed(2)
                  : '0';
        ...
        pw.Container(
          ...
          child: pw.Row(
            ...
            children: [
              ...
              pw.Text(
                '\$${order['total_balance']?.toString() ?? '0'}',
                style: pw.TextStyle(
                  fontSize: 16,
                  fontWeight: pw.FontWeight.bold,
                  color: PdfColors.blue900,
