...
class AddStorageStaffAccountPopup extends StatefulWidget {
  ...
  const AddStorageStaffAccountPopup({super.key, this.onAccountCreated});

  @override
  State<AddStorageStaffAccountPopup> createState() =>
      _AddStorageStaffAccountPopupState();
}

class _AddStorageStaffAccountPopupState
    extends State<AddStorageStaffAccountPopup> {
  ...
  void initState() {
    super.initState();
    // Auto-fill Account ID when Storage Staff ID changes
    idCtrl.addListener(() {
  ...
  Future<void> _submitAccount() async {
    ...
    if (idCtrl.text.trim().isEmpty || idCtrl.text.trim().length != 9) {
      setState(() => idError = 'Please enter a valid 9-digit Storage Staff ID');
      hasError = true;
    }

    if (nameCtrl.text.trim().isEmpty) {
      setState(() => nameError = 'Please enter storage staff name');
      hasError = true;
    }
    ...
    try {
      final staffId = int.parse(idCtrl.text.trim());

      // Check if storage staff ID already exists
      final existingStaff = await supabase
          .from('storage_staff')
          .select('storage_staff_id')
          .eq('storage_staff_id', staffId)
          .maybeSingle();
      ...
      if (existingStaff != null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('This Storage Staff ID already exists'),
            ),
          );
        }
        return;
      ...
      // Insert storage staff
      await supabase.from('storage_staff').insert({
        'storage_staff_id': staffId,
      ...
      // Insert user account
      await supabase.from('user_account_storage_staff').insert({
        'storage_staff_id': staffId,
...
void showAddStorageStaffAccountPopup(
  ...
  showDialog(
    ...
    builder: (_) =>
        AddStorageStaffAccountPopup(onAccountCreated: onAccountCreated),
  );
}
