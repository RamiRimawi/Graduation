...
class _DeliveryArchiveState extends State<DeliveryArchive> {
  int _selectedIndex = 1; // Archive tab
  ...
  Future<void> _fetchDeliveredOrders() async {
    ...
    try {
      ...
      for (final row in data) {
        ...
        final orderDescription = row['customer_order_description'] as List<dynamic>? ?? [];
      ...
      fetched.sort((a, b) {
        final da = (a['deliveredAt'] as DateTime?) ?? (a['orderDate'] as DateTime?);
        final db = (b['deliveredAt'] as DateTime?) ?? (b['orderDate'] as DateTime?);
